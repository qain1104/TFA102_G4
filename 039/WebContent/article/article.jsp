					...
					<div class="card col-lg-12 article-card">
						...
						<div class="container mt-4">
							<div
								...
								<div class="col-auto">
									<c:choose>
										<c:when test="${param.whichClass==1}"><p class="fs-1 fw-bold text-success">運動休閒</p></c:when>
										<c:when test="${param.whichClass==2}"><p class="fs-1 fw-bold text-success">商品分享</p></c:when>
										<c:when test="${param.whichClass==3}"><p class="fs-1 fw-bold text-success">運動賽事</p></c:when>
										<c:when test="${param.whichClass==4}"><p class="fs-1 fw-bold text-success">我的文章</p></c:when>
										<c:otherwise><p class="fs-1 fw-bold text-success">論壇首頁</c:otherwise>
									</c:choose>

								</div>
								<div class="col-auto">
									<div class="input-group mb-3">
										<input type="text" class="form-control" placeholder="文章搜尋"
								...
								<c:forEach var="article" items="${list}" begin="<%=pageIndex%>"
									...
									<a href="#" class="list-group-item list-group-item-action">
										<div class="row align-items-center">
											...
											<div class="col-2" style="font-size: 8px;">${article.articleDate}</div>
										</div>
									</a>
								</c:forEach>
								<!-- close文連結列表 -->
